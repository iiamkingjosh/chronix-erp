...
| D9 (new) | `sendPushToTokens()` result handling | A failed delivery is detectable, not assumed successful because nothing threw | `sendPushToTokens()`'s return type was changed from `Promise<void>` to a result object (`{successCount, failureCount, errorCodes}`) as part of fixing D1 (this session), since the function never throws on delivery failure — it always resolves, even for malformed/invalid tokens. The new `/api/notifications/push` route correctly checks this result. 5 pre-existing callers (4 cron routes + `notifications/test`) still just `await` and ignore it, carrying the same silent-failure exposure D1 had before this fix. Deferred — not fixed in this pass. |
