...
| D9 (new, accepted risk) | `isTeamMemberTaskUpdate()` status guard | `status` should only ever ride alongside a genuine `tasks`+`progress` write, never alone, so a non-manager can never set status directly outside of what `deriveProjectStatus()` itself computed | `isTeamMemberTaskUpdate()`'s status field lacks a guard ensuring it only rides alongside a genuine `tasks`+`progress` write, due to an unexplained Firestore rules parser issue specifically triggered by checking `affectedKeys()` against the key name `'progress'` (confirmed: the identical check against `'tasks'` in the identical position works fine; 5+ variations tried — `hasAny`, `hasAll`, `'in'` membership, with/without shared `let` bindings — all fail identically for `'progress'` specifically). Severity: low — exploit requires an internal staff member bypassing the UI to hand-craft a raw Firestore write; affects only that project's own status display; no financial or cross-tenant exposure. Real application code (`addTask()`/`setTaskStatus()`) always sends the fields together correctly. Deferred as its own investigation, not blocking the status auto-derivation feature this finding was discovered while building. |
