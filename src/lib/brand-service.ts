...
export async function getBrandAssets(): Promise<BrandAsset[]> {
  const snap = await getDocs(query(collection(db, "brand_assets"), orderBy("createdAt", "desc"), limit(50)));
...
export async function getContentPosts(): Promise<ContentPost[]> {
  const snap = await getDocs(query(collection(db, "content_posts"), orderBy("scheduledDate", "asc"), limit(50)));
...
export async function getEmailCampaigns(): Promise<EmailCampaign[]> {
  const snap = await getDocs(query(collection(db, "email_campaigns"), orderBy("createdAt", "desc"), limit(50)));
