...
/* Use configured FROM address, fall back to Resend's verified test address
   (only relevant if RESEND_FROM_EMAIL is ever unset — chronixtechnology.com
   itself is verified domain-wide, confirmed directly via Resend's API on
   2026-06-24, so any address at that domain works with no further setup). */
...
/* ── Email templates ─────────────────────────────────────────── */
/* Single shared white/light shell, extracted from invoiceEmail()'s
 * existing, proven markup below (same header band, same footer) rather
 * than inventing a new design. Every template except invoiceEmail() now
 * uses this — see the note above invoiceEmail() for why it stays
 * independent. The old dark shell()/bodyRow()/disclaimer() have no
 * remaining callers and have been removed. */

function whiteShell(badge: string, content: string): string {
...
<table width="580" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
  <tr><td style="background:#003366;padding:28px 32px;">
    ...
    <p style="margin:6px 0 0;color:rgba(255,255,255,0.6);font-size:11px;letter-spacing:0.2em;text-transform:uppercase;">${badge}</p>
  </td></tr>
  ${content}
...
function whiteBodyRow(title: string, message: string, link: string, btnLabel: string): string {
  return `<tr><td style="padding:32px;">
    <p style="margin:0 0 8px;color:#111;font-size:18px;font-weight:600;">${title}</p>
    <p style="margin:0 0 28px;color:#555;font-size:14px;line-height:1.7;">${message}</p>
...
function whiteDisclaimer(text: string): string {
  return `<tr><td style="padding:0 32px 24px;">
    <p style="margin:0;color:#999;font-size:11px;">${text}</p>
  </td></tr>`;
}
...
export function taxReminderEmail(title: string, message: string, link: string): string {
  return whiteShell(
    "Tax Notification",
    whiteBodyRow(title, message, link, "Open Tax Dashboard") +
    whiteDisclaimer("All tax figures are estimates only — always verify with a qualified tax advisor before filing with FIRS/LIRS."),
  );
}
...
export function taxActivityEmail(title: string, message: string, link: string): string {
  return whiteShell(
    "Tax Activity",
    whiteBodyRow(title, message, link, "View Details") +
    whiteDisclaimer("This notification was triggered by a recorded action in Chronix Technology Limited."),
  );
}
...
export function subscriptionAlertEmail(title: string, message: string, link: string): string {
  return whiteShell(
    "Subscription Alert",
    whiteBodyRow(title, message, link, "View Subscription"),
  );
}
...
export function welcomeEmail(displayName: string, role: string): string {
  return whiteShell(
    ...
    `<tr><td style="padding:32px;">
      <p style="margin:0 0 8px;color:#111;font-size:18px;font-weight:600;">Hello ${displayName},</p>
      <p style="margin:0 0 16px;color:#555;font-size:14px;line-height:1.7;">Welcome to Chronix Technology Limited! Your account has been successfully created, and you're now ready to access the Chronix platform.</p>
      <p style="margin:0 0 24px;color:#555;font-size:14px;line-height:1.7;">As a valued member of our team, you can sign in to manage your activities, access company resources, collaborate with colleagues, and stay updated with important information.</p>
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:28px;">
        <tr>
          <td style="padding:14px 18px;border-bottom:1px solid #e2e8f0;">
            <p style="margin:0;color:#888;font-size:10px;text-transform:uppercase;letter-spacing:0.08em;">Account Status</p>
            <p style="margin:2px 0 0;color:#111;font-size:13px;font-weight:600;">Active</p>
          </td>
        </tr>
        <tr>
          <td style="padding:14px 18px;">
            <p style="margin:0;color:#888;font-size:10px;text-transform:uppercase;letter-spacing:0.08em;">Role</p>
            <p style="margin:2px 0 0;color:#111;font-size:13px;font-weight:600;">${role}</p>
          </td>
        </tr>
      </table>
      <p style="margin:0 0 8px;color:#111;font-size:13px;font-weight:600;text-transform:uppercase;letter-spacing:0.06em;">Get Started</p>
      ...
      <p style="margin:24px 0 16px;color:#555;font-size:14px;line-height:1.7;">If you experience any issues accessing your account, please contact the Chronix Technology support team for assistance.</p>
      <p style="margin:0 0 20px;color:#555;font-size:14px;line-height:1.7;">We are excited to have you on board and look forward to achieving great things together.</p>
      <p style="margin:0;color:#555;font-size:14px;line-height:1.7;">Best regards,<br>Chronix Technology Limited</p>
    </td></tr>`,
  );
}
 ...
 * it's used as-is here instead of via whiteBodyRow() (which prefixes APP_URL). */
export function passwordResetEmail(link: string): string {
  return whiteShell(
    "Password Reset",
    `<tr><td style="padding:32px;">
      <p style="margin:0 0 8px;color:#111;font-size:18px;font-weight:600;">Reset your password</p>
      <p style="margin:0 0 28px;color:#555;font-size:14px;line-height:1.7;">Follow this link to choose a new password for your Chronix ERP account. If you didn't request this, you can ignore this email.</p>
...
/** invoiceEmail() stays independent rather than migrating to whiteShell():
 * its content shape is genuinely different (a 3-column line-item table
 * plus a separate bank-details table, not a single title+message+button),
 * and its footer is already deliberately different (TIN/website, no "do
 * not reply" line — appropriate for a billing document, less so for a
 * password reset or welcome email if whiteShell() ever changes). Forcing
 * it through the shared shell would mean reconciling two different footer
 * intents under one component for no real duplication savings, since its
 * wrapper is the only thing shared and that's now a two-line whiteShell()
 * call away regardless. Kept as-is per the explicit "no change needed." */
